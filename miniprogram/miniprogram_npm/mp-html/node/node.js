      ...
      observer (nodes) {
        ...
        function diff (a, b, path) {
          ...
          for (let i = 0; i < Math.min(a.length, b.length); i++) {
            const prevItem = asObject(a[i])
            const nextItem = asObject(b[i])
            const keys = new Set(Object.keys(prevItem).concat(Object.keys(nextItem)))
            for (const key of keys) {
              if (prevItem[key] === undefined || nextItem[key] === undefined || (typeof nextItem[key] !== 'object' && prevItem[key] !== nextItem[key])) {
                data[path + '[' + i + ']'] = nextItem
                break
              }
            }
            if (!((path + '[' + i + ']') in data)) {
              for (const key of keys) {
                ...
                if (Array.isArray(nextValue)) {
                  diff(prevValue, nextValue, path + '[' + i + '].' + key)
                } else if (nextValue && typeof nextValue === 'object') {
                  const prevObject = asObject(prevValue)
                  const nextObject = asObject(nextValue)
                  for (const subKey of Object.keys(prevObject).concat(Object.keys(nextObject))) {
                    if (prevObject[subKey] !== nextObject[subKey]) {
                      data[path + '[' + i + '].' + key] = nextObject
                      break
                    }
                  }
                }
        ...
        diff(this.data.nodes, asArray(nodes), 'nodes')
